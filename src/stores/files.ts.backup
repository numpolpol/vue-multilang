...
export const useFilesStore = defineStore('files', {
  state: () => ({
    ...
    // No dual-key mode needed - iOS only
  }),
  ...
  actions: {
    ...
    async uploadFileToLanguage(languageCode: string, file: File, fileType: 'strings') {
    ...
    syncLanguagesToFiles() {
      ...
      // No dual-key processing needed for iOS-only mode
    },
      ...
      if (this.currentProject) {
        ...
        if (this.hasLanguageFiles) {
          ...
          this.currentProject.languages = this.files.map((file, index) => ({
            name: file.name.replace(/\.strings$/, ''),
